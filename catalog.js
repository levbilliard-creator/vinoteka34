...
s = s.replace(/\b(сухое|полусухое|полусладкое|сладкое|брют|экстра\s*брют|экстра\s*драй|экстра\s*сухое)\s+(белое|красное|розовое)/ig,'');

s = s.replace(/\b(белое|красное|розовое)\b/ig,'');

s = s.replace(/\b(сухое|полусухое|полусладкое|сладкое|брют|экстра\s*брют|экстра\s*драй|экстра\s*сухое)\b/ig,'');
...
if(src) return cleanTitle(src);
return cleanTitle(toLatin(p && p.title ? p.title : ""));
}
...
const enTitle = cleanTitle((item.en || item.title_en || "").trim() || titleEN(item));
